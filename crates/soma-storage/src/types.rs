...
pub struct AuditEvent {
    ...
    pub seq_num: i64,
    /// Event type (e.g. "secret.write", "token.create").
    pub event_type: String,
    /// Token ID of the actor (None for system events).
    pub actor_token_id: Option<Uuid>,
    /// Role of the actor.
    pub actor_role: Option<String>,
    /// Resource type (e.g. "secret", "config", "project").
    pub resource_type: Option<String>,
    /// Resource identifier (path or UUID string).
    pub resource_id: Option<String>,
    /// Outcome of the operation.
    pub outcome: String,
    /// IP address of the actor if available.
    pub actor_ip: Option<String>,
    /// HMAC of the previous entry (None for seq 1).
    pub prev_hash: Option<String>,
    /// HMAC of this entry's canonical fields.
...
pub struct AuditFilters {
    ...
    /// Return only events at or before this time.
    pub to: Option<DateTime<Utc>>,
    /// Filter by `actor_token_id`.
    pub actor: Option<Uuid>,
    /// Pagination limit.
    pub limit: i64,
    /// Keyset cursor: the last `seq_num` from the previous page (descending).
...
pub struct AuditVerifyResult {
    ...
    /// The first `seq_num` where a broken link was found, if any.
